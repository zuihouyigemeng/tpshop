...
class  Template {
    ...
    private function parseTemplateName($templateName){
		...
		if(substr($templateName,0,2)=='{$'){
			preg_match_all("/".$this->config['tmpl_begin']."(.*)".$this->config['tmpl_end']."(.*)/is",$templateName,$linb);
            $templateName = ltrim(str_replace(__ROOT__,'',$this->get(substr($linb[1][0],1))),'/');
			$templateName.=$linb[2][0];
		}		
